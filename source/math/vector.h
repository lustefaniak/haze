...
struct vec3_t
{
   ...
   void Set(float val) {v[0] = val; v[1] = val; v[2] = val;}
...
inline vec3_t vec3_t::Cross(const vec3_t &p){
		vec3_t temp;

/*		temp[0] = v[1] * p[2] - v[2] * p[1];
		temp[1] = v[2] * p[0] - v[0] * p[2];
		temp[2] = v[0] * p[1] - v[1] * p[0];*/

	temp[0] = (p.v[1] * v[2]) - (p.v[2] * v[1]);
	temp[1] = (p.v[2] * v[0]) - (p.v[0] * v[2]);
	temp[2] = (p.v[0] * v[1]) - (p.v[1] * v[0]);
		return temp;
//return 0;
}

inline void vec3_t::PackTo01()
{
	(*this)=GetPackedTo01();	
}

inline vec3_t vec3_t::GetPackedTo01() const
{
	vec3_t temp(*this);

	temp.Normalize();

	temp=temp*0.5f+vec3_t(0.5f, 0.5f, 0.5f);
